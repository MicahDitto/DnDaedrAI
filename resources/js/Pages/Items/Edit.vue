...
const form = useForm({
    ...
    content: {
        description: props.item.content?.description || '',
        properties: props.item.content?.properties || '',
        history: props.item.content?.history || '',
        secrets: props.item.content?.secrets || '',
    },
    owner_id: props.currentOwnerId || '',
    location_id: props.currentLocationId || '',
    confidence: props.item.confidence,
    is_secret: props.item.is_secret,
});

const submit = () => {
    form.put(route('campaigns.items.update', [props.campaign.slug, props.item.slug]));
};
</script>

<template>
    <Head :title="`Edit ${item.name} - ${campaign.name}`" />
                ...
                <div class="bg-gunmetal overflow-hidden shadow-dark-md sm:rounded-lg border border-arcane-periwinkle/10">
                    <div class="p-6">
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <!-- Details Section -->
                            <div class="border-b border-charcoal/50 pb-6">
                                <h3 class="text-lg font-medium text-white mb-4">Item Details</h3>
