...
const form = useForm({
    ...
    subtype: props.religion.subtype,
    summary: props.religion.summary || '',
    content: {
        description: props.religion.content?.description || '',
        beliefs: props.religion.content?.beliefs || '',
        practices: props.religion.content?.practices || '',
        hierarchy: props.religion.content?.hierarchy || '',
        symbols: props.religion.content?.symbols || '',
        holy_sites: props.religion.content?.holy_sites || '',
        history: props.religion.content?.history || '',
        relationships: props.religion.content?.relationships || '',
        taboos: props.religion.content?.taboos || '',
        afterlife: props.religion.content?.afterlife || '',
        secrets: props.religion.content?.secrets || '',
    },
    headquarters_id: props.currentHeadquartersId || '',
    founded_by_id: props.currentFounderId || '',
    confidence: props.religion.confidence,
    is_secret: props.religion.is_secret,
});

const submit = () => {
    form.put(route('campaigns.religions.update', [props.campaign.slug, props.religion.slug]));
};
</script>

<template>
    <Head :title="`Edit ${religion.name} - ${campaign.name}`" />
