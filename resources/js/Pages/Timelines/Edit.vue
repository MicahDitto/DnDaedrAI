...
const form = useForm({
    ...
    subtype: props.timeline.subtype,
    summary: props.timeline.summary || '',
    content: {
        description: props.timeline.content?.description || '',
        start_date: props.timeline.content?.start_date || '',
        end_date: props.timeline.content?.end_date || '',
        key_events: props.timeline.content?.key_events || '',
        duration: props.timeline.content?.duration || '',
        significance: props.timeline.content?.significance || '',
        secrets: props.timeline.content?.secrets || '',
    },
    confidence: props.timeline.confidence,
    is_secret: props.timeline.is_secret,
});

const submit = () => {
    form.put(route('campaigns.timelines.update', [props.campaign.slug, props.timeline.slug]));
};
</script>

<template>
    <Head :title="`Edit ${timeline.name} - ${campaign.name}`" />
        ...
        <div class="py-6">
            <div class="max-w-3xl mx-auto">
                ...
                <div class="mb-6 flex items-center space-x-4">
                    <Link
                        :href="route('campaigns.timelines.show', [campaign.slug, timeline.slug])"
