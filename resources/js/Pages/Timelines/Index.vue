...
const filteredTimelines = computed(() => {
    if (!filterSubtype.value) return props.timelines;
    return props.timelines.filter(t => t.subtype === filterSubtype.value);
});

const getSubtypeColor = (subtype: string) => {
    const colors: Record<string, string> = {
        age: 'bg-arcane-purple/20 text-arcane-purple',
        epoch: 'bg-legendary-gold/20 text-legendary-gold',
        dynasty: 'bg-arcane-periwinkle/20 text-arcane-periwinkle',
        period: 'bg-nature/20 text-nature',
        event: 'bg-legendary-amber/20 text-legendary-amber',
        calendar: 'bg-arcane-lavender/20 text-arcane-lavender',
        chronicle: 'bg-arcane-purple/30 text-arcane-purple',
        annals: 'bg-nature/30 text-nature',
    };
...
<template>
    <Head :title="`History & Timeline - ${campaign.name}`" />
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto">
                ...
                <div class="mb-6 flex items-center justify-between">
                    ...
                    <Link
                        :href="route('campaigns.timelines.create', campaign.slug)"
                ...
                >
                    <div class="p-12 text-center">
                        ...
                        <h3 class="mt-4 text-lg font-medium text-white">No timelines yet</h3>
                        <p class="mt-2 text-sm text-arcane-grey">
                            Track the ages, eras, and events that shape your world's history.
                        </p>
                        <div class="mt-6">
                            <Link
                                :href="route('campaigns.timelines.create', campaign.slug)"
                ...
                <div
                    v-else-if="filteredTimelines.length === 0"
                ...
                >
                    <Link
                        v-for="item in filteredTimelines"
                        :key="item.id"
                        :href="route('campaigns.timelines.show', [campaign.slug, item.slug])"
